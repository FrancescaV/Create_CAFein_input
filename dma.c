...
/* The functions from the first two programs are made into their own c file. There are
a few modifications. Some function names are changed to avoid confusion. Also, for every
dimension, the arrays are made one entry bigger to allow for an empty, unused slot
with index zero */
